...
class UserController extends AbstractController
{
    ...
    {
        $user = new User(); // Par défaut, on crée un objet générique de type User.
        $form = $this->createForm(UserType::class, $user);
        $form->handleRequest($request);
    
        // Vérification des rôles en fonction du type d'utilisateur connecté
        $currentUser = $this->getUser();
        if ($currentUser instanceof ResponsableSalle) {
            // Si le Responsable de salle, il peut seulement ajouter des Sportif ou Entraineur
            $form->get('role')->addViewTransformer(new CallbackTransformer(
                function ($originalRole) {
                    return Role::SPORTIF->value; // Par défaut, il n'y a que Sportif dans la liste
                },
                function ($submittedRole) {
